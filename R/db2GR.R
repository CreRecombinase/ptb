...
gwas_df <- dplyr::tbl(dplyr::src_sqlite(path=input_db,create=F),"gwas") %>% collect()
gwas_z <- dplyr::select(gwas_df,SNP=id,chrom,pos,beta=!!beta_v,se=!!se_v) %>%
    dplyr::mutate(`z-val`=beta/se)


m_gwas_df <- select(gwas_df,chrom,pos,MAJOR=A1,MINOR=A2) %>%
    mutate(chrom=as.integer(chrom),pos=as.integer(pos),SNP=paste0(chrom,":",pos))
...
snp_df <- gwas_z %>%
    ...
    arrange(chr,pos) %>%
    ldshrink::assign_snp_block(break_df=mutate(ld_df,chr = as.integer(gsub("chr","",chrom))), assign_all=T)
snp_df <- dplyr::group_by(snp_df, chr, region_id) %>%
    dplyr::summarise(ct=n()) %>%
    dplyr::ungroup() %>%
    dplyr::inner_join(snp_df) %>%
    dplyr::mutate(region_id = ifelse(ct<min_size, region_id-1, region_id)) %>%
    dplyr::select(-ct) %>%
    dplyr::ungroup()
...
GR_df <- select(gwas_z,chrom,start=pos) %>%
    mutate(chrom=paste0("chr",chrom),start=as.integer(start),end=start+1L) %>%
